...
CREATE TABLE jeux._utilisateur (
    identifiant VARCHAR(20) NOT NULL,
    nom VARCHAR(20),
    prenom VARCHAR(20),
    motDePasse VARCHAR(20),
    droit BOOLEAN DEFAULT false,
    CONSTRAINT _utilisateur_pk PRIMARY KEY (identifiant)
);


CREATE TABLE jeux._collection (
    id_collectionneur VARCHAR(20),
    id_jeu INTEGER
);
...
PRIMARY KEY ( id_jeu,id_collectionneur );


ALTER TABLE jeux._collection
ADD CONSTRAINT FK1_collection
FOREIGN KEY (id_jeu) references jeux._jeu(id) ON DELETE CASCADE;


ALTER TABLE jeux._collection
ADD CONSTRAINT FK2_collection
FOREIGN KEY (id_collectionneur) references jeux._utilisateur(identifiant) ON DELETE CASCADE;



/*--- trigger -----*/
create or replace function jeux._doublon() returns trigger as
$$
Begin
  if exists ( select id_jeu from jeux._collection where id_collectionneur = new.id_collectionneur and id_jeu = new.id_jeu) then
      delete from jeux._collection where id_collectionneur = new.id_collectionneur and id_jeu = new.id_jeu;
  end if;
  return new;
  end;
  $$ language plpgsql;

create trigger doublon
BEFORE INSERT 
ON jeux._collection
for each row execute procedure jeux._doublon();



create or replace function jeux.cinq_jeux() returns TRIGGER as 
$$
 DECLARE
  nbjeux integer;/* nb de jeux present dans la table*/
  BEGIN
    select count(id_jeu) from jeux._collection 
    where id_collectionneur = new.id_collectionneur
    into nbjeux;
    if nbjeux >= 5  then /*- si jeux > 5 alors on efface le dernier jeux ajouter*/
      delete  from jeux._collection 
      where id_collectionneur = new.id_collectionneur
      and id_jeu in 
      ( select id_jeu from jeux._collection where id_collectionneur = new.id_collectionneur and date_ajout = ( select MAX(date_ajout)
      from jeux._collection where id_collectionneur = new.id_collectionneur group by id_jeu limit 1 ) limit 1);
    end if;
    return new;
   end;
 $$ language plpgsql;  
...
create trigger cinq_jeux
BEFORE INSERT 
ON jeux._collection
for each row execute procedure jeux.cinq_jeux();
